...
def build_public_library(
    ...
    for canonical_id, examples in groups.items():
        ...
        style_item = {
            ...
            "palette": curator.get("palette") or [],
            "palette_note_pt": curator.get("palette_note_pt"),
        ...
        for entry in collection.get("entries") or []:
            ...
            if target is None:
                ...
                target = {
                    ...
                    "palette_note_pt": entry.get("palette_note_pt"),
                    "observed_in_pt": entry.get("observed_in_pt"),
                    "reading": public_reading(entry.get("reading")),
                    "family": family,
                    "family_label": family_labels.get(family, family),
                    "tags": tags,
                    "tag_labels": [tag_labels.get(tag, tag) for tag in tags],
                    "naming_basis": entry.get("naming_basis") or "source_name",
                    "confidence": "high",
                    "poster_url": None,
                    "example_count": 0,
                    "recipe_count": 0,
